    ...
    def update(self):
        ...
        if self.game.renderingEnabled:
            for tile in toRender:
                ...
                if x-self.game.camera.x > -self.tileWidth*1.2 and y-self.game.camera.y > -self.tileHeight*1.2:
                    if x-self.game.camera.x < self.game.window.w*1.1 and y-self.game.camera.y < self.game.window.h*1.1:
                        img = self.imgGrass
                        if tileType == 0:
                            img = self.imgGrass
                        if tileType == 1:
