...
class Bob:
    ...
    def random_move(self):
        ...
        if not self.stayOnFood:
            self.energy -= self.mass * (self.velocity ** 2) # + self.perception/20
        else:
    ...
    def reproduce(self):
        ...
        # mutated genetics
        randVel = random.randint(int(self.velocity*10)-1, int(self.velocity*10)+4)
        newBob.velocity = clamp(randVel, self.game.tilemap.minVelocity*10, self.game.tilemap.maxVelocity*10)/10
        randMass = random.randint(int(self.mass*10)-1, int(self.mass*10)+4)
        newBob.mass = clamp(randMass, self.game.tilemap.minMass*10, self.game.tilemap.maxMass*10)/10
        randPerc = random.randint(int(self.perception)-1, int(self.perception)+6)
        newBob.perception = clamp(randPerc, self.game.tilemap.minPerc, self.game.tilemap.maxPerc)
