...
class Game:
    def __init__(self):
        ...
        # use GPU acceleration? (uses 'pygame-ce' = pygame community edition)
        # pygame-ce install: pip uninstall pygame && pip install pygame-ce
    ...
    # property function to return the sdl2 renderer (gpu accel)
    @property
    def renderer(self):
    ...
    def init(self):
        ...
        # file system
        # self.savefile.load_file("test.save")
        self.autosaveInterval = 250
        self.autosaveTimer = Timer(self, self.autosaveInterval, self.autosave, True)
        self.add_timer_not_tick(self.autosaveTimer)
    ...
    def update(self):
        ...
        if self.renderingEnabled:
            ...
            if not self.isDay:
                if not self.gpuAcceleration:
                    self.screen.blit(self.colorFilterSurface, (0, 0))
                else:
                    tex = sdl2.Texture.from_surface(self.renderer, self.colorFilterSurface)
                    tex.draw(dstrect=(0, 0))
        ...
        if self.dayTimer.current <= self.dayTimer.duration/5:
            # remove all food
            self.tilemap.foods = []
